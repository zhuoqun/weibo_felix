$(document).ready(function (){
  ...
  function delElements(arr) {
    for (var k in arr) {
      $(arr[k]).remove();
    }
  }

  function delElementsLater(arr) {
    for (var k in arr) {
      $(arr[k]).remove();
    }
  }
  ...
  var home_del_later = [
    ...
    //'.W_layer[node-type="outer"]', // 各种弹窗
  ];

  delElements(home_del_now);

  setInterval(function(){
    delElements(home_del_later);
  }, 1000); 
